...
export function AdminWelcomeCard() {
  ...
  useEffect(() => {
    ...
    leaveRequestsApi
      ...
      .then((data) => {
        if (Array.isArray(data.leaveRequests)) {
          setPendingRequests(
            data.leaveRequests.filter((r: any) => r.status === "pending").length
          );
    ...
    PerformanceApi.getUpcomingReviews()
      .then((data) => {
        if (Array.isArray(data)) {
      ...
      <CardContent className="p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-indigo-200 mt-1 font-mono">
              {currentTimeString}
            </p>
            <p className="text-indigo-200 mt-2">
              You have {pendingRequests !== null ? pendingRequests : "-"}{" "}
              pending leave requests and{" "}
              {teamMembers !== null ? teamMembers : "-"} team members.
            </p>
          </div>
