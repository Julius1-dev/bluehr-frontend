...
export async function fetchMasterPayroll(
  payrollMonth: number | string,
  payrollYear: number,
...
export function PayrollProcessor({
  ...
  const [selectedFrequency, setSelectedFrequency] =
    useState<PaymentFrequencyFilter>("all");
  ...
  const [_checkingStatus, setCheckingStatus] = useState<boolean>(false);
  ...
  const checkPayrollStatus = async () => {
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/company-admin/payroll/status?payrollMonth=${getMonthNumber(
          payrollMonth
        )}&payrollYear=${payrollYear}&paymentFrequency=${selectedPaymentFrequency}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  useEffect(() => {
    const fetchDepartmentsAndEmployees = async () => {
      try {
        ...
        if (!deptResponse.ok) {
          throw new Error(
            `Failed to fetch departments: ${deptResponse.status} ${deptResponse.statusText}`
          );
        }
        ...
        if (!empResponse.ok) {
          throw new Error(
            `Failed to fetch employees: ${empResponse.status} ${empResponse.statusText}`
          );
        }
        ...
        if (Array.isArray(empData)) {
          const employeeData: Employee[] = empData
            ...
            .map((emp: any) => {
              ...
              const totalDeductions =
                paye + shif + nssfContribution + housingLevy + deductions;
              const netPay = adjustedGrossSalary - totalDeductions;

              const departmentName =
                deptMap[emp.department_id] || "Unknown Department";
              ...
              if (actualPaymentFrequency === "yearly") {
                ...
                adjustedPaye = calculatePAYE(
                  adjustedGrossSalary,
                  adjustedNssfContribution
                );
              ...
              return {
                ...
                net_pay:
                  actualPaymentFrequency === "yearly" ? adjustedNetPay : netPay,
                status: "pending",
          ...
          const mergedEmployees = employeeData.map((emp) => ({
            ...
            earlyLeaveDeduction:
              attendanceById[emp.id]?.earlyLeaveDeduction || 0,
          }));
  ...
  const handleProcessPayroll = async () => {
    ...
    if (payrollStatus.status === "processed") {
      toast.error("Payroll Already Processed", {
        description: `Payroll for ${payrollMonth} ${payrollYear} (${selectedPaymentFrequency}) has already been processed successfully on ${new Date(
          payrollStatus.processedAt!
        ).toLocaleDateString()}`,
        duration: 5000,
      });
      return;
    ...
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/company-admin/payroll/process`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Failed to process payroll: ${response.status} ${response.statusText}`
        );
      }
      ...
      if (data.success) {
        ...
        const updatedEmployees = employees.map((emp) => {
          const wasProcessed = filteredEmployeesList.some(
            (filtered) => filtered.id === emp.id
          );
          return {
            ...emp,
        ...
        toast.success("Payroll Processed Successfully!", {
          description: `Total amount: ${formatCurrency(data.totalAmount)} • ${
            data.employeeCount
          } employees processed`,
          duration: 5000,
        });
        ...
        setError(
          "Failed to process payroll: " + (data.error || "Unknown error")
        );
      }
    } catch (error) {
      ...
      if (errorMessage.includes("already been processed successfully")) {
        toast.error("Payroll Already Processed", {
          description:
            "This payroll period has already been processed successfully and cannot be processed again.",
          duration: 5000,
        });
        // Refresh status
        checkPayrollStatus();
        ...
        toast.error("Insufficient Balance", {
          description:
            "Your company wallet does not have enough funds to process payroll.",
          duration: 5000,
        });
      } else {
      ...
      if (response.ok) {
        ...
        if (data.reports && data.reports.length > 0) {
          ...
          const currentReport = data.reports.find(
            (report: any) =>
              report.report_period.includes(payrollMonth) &&
              report.report_period.includes(payrollYear.toString())
          );
      ...
      const matchesFrequency =
        ...
        (selectedFrequency === "monthly" &&
          (employee.payment_frequency === "monthly" ||
            employee.payment_frequency === "yearly")) ||
    ...
    <Card>
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            ...
            {filteredEmployeesList.length > 0 && (
              <span className="ml-2">
                • Total Payroll: {formatCurrency(totalPayrollAmount)}
              </span>
            )}
            {payrollStatus.status === "processed" &&
              payrollStatus.processedAt && (
                <span className="ml-2">
                  • Processed:{" "}
                  {new Date(payrollStatus.processedAt).toLocaleDateString()}
                </span>
              )}
          </p>
        </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <Select
                  ...
                  onValueChange={setSelectedPaymentFrequency}
                >
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSelectedFrequency(
                      e.target.value as PaymentFrequencyFilter
                    )
                  }
                >
                  ...
                  {filteredEmployeesList.length > 0 ? (
                    filteredEmployeesList.map((employee) => (
                      <TableRow key={employee.id}>
                        ...
                        <TableCell className="text-red-600">
                          {formatCurrency(employee.shif)}
                        </TableCell>
                        <TableCell className="text-red-600">
                          {formatCurrency(employee.nssf)}
                        </TableCell>
                        <TableCell className="text-red-600">
                        ...
                        <TableCell className="text-red-600">
                          {formatCurrency(
                            (employee.latenessDeduction || 0) +
                              (employee.earlyLeaveDeduction || 0)
                          )}
                        </TableCell>
      ...
      {isEditModalOpen && selectedEmployee && (
        <EditPayrollModal
          employee={{
            ...selectedEmployee,
            payment_frequency: selectedEmployee.payment_frequency as
              | "hourly"
              | "weekly"
              | "monthly"
              | "yearly",
