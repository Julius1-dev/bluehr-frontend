...
export default function PayrollExemptionsModal({
  open,
  onClose,
}: PayrollExemptionsModalProps) {
    ...
    const fetchRecords = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${BACKEND_URL}/company-admin/payroll/attendance-deductions?${params.toString()}`,
  ...
  const handleExempt = async (recordId: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await fetch(
        `${BACKEND_URL}/company-admin/master-payroll/attendance-records/${recordId}/exempt`,
        {
          method: "PATCH",
          headers: {
            ...
            <div className="w-full max-h-[60vh] overflow-auto">
              {loading ? (
                ...
                <div className="text-center py-8">
                  No attendance deductions found.
                </div>
                ...
                <table className="min-w-full text-sm border-collapse">
                  ...
                  <tbody>
                    {records.map((record) => (
                      <tr
                        key={record.id}
                        className={record.exempted ? "bg-green-50" : ""}
                      >
                        ...
                        <td className="p-2 whitespace-nowrap">
                          {record.exempted
                            ? `Exempted (${record.exemptionReason})`
                            : "Active"}
                        </td>
                        <td className="p-2 whitespace-nowrap">
                          {!record.exempted && (
                            <Button
                              size="sm"
                              onClick={() => handleExempt(record.id)}
                            >
                              Exempt
                            </Button>
                          )}
