...
import { Check, Clock, MapPin, StopCircle } from 'lucide-react';
...
export default function EmployeeCheckInOut() {
  ...
  const fetchData = async () => {
   ...
   try {
    ...
    if (axios.isAxiosError(err)) {
      console.error('No shift assigned today:', err.response?.data?.error || err.message);
    } else {
      console.error('No shift assigned today:', (err as Error).message || 'Unknown error');
    }
  ...
  const isWithinOffice = selectedLocation && currentCoords
    ? isWithinRadius(
        currentCoords.lat,
        currentCoords.lng,
        ...
        <CardContent className="space-y-4">
          <div className="space-y-4">
            ...
            {selectedLocation && currentCoords && (
              <p className="text-sm">
                You are {isWithinOffice ? '✅ within' : '❌ outside'} the allowed radius of "{selectedLocation.name}"
              </p>
            )}
          </div>
          ...
          {currentCoords && (
            <div className="flex items-center gap-3 text-sm text-gray-700">
              ...
              <span>Current position: Lat: {currentCoords.lat.toFixed(5)}, Lng: {currentCoords.lng.toFixed(5)}</span>
            </div>
          )}

          {isCheckedIn && checkInTime ? (
            <div className="space-y-2">
              <p className="text-sm text-gray-600">Checked in at: {checkInTime.toLocaleTimeString()}</p>
              <p className="text-sm text-gray-600">Elapsed Time: {elapsedTime}</p>
              ...
              <Button onClick={handleCheckOut} className="gap-2 bg-red-600 hover:bg-red-700">
            ...
            className="gap-2"
            disabled={!isWithinOffice || !selectedLocation || !assignedShift || loadingShift}
          >
