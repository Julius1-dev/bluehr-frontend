...
export function Documents() {
  ...
  const [_recipientOptions] = useState([
  ...
  const filteredDepartments: Department[] = departments.filter((d) =>
    d.name.toLowerCase().includes(shareSearch.toLowerCase())
  );
  ...
  const getDocumentsForView = () => {
    if (isAdmin) {
      ...
      return filteredDocuments.filter(
        (doc) => doc.is_shared || doc.uploaded_by === currentUserId
      );
    }
  };
  ...
  const _userDepartments = employees.find((e) => e.id === currentUserId)
    ?.department_id
    ? [employees.find((e) => e.id === currentUserId)?.department_id]
    : [];
  ...
  function isSharedWith(doc: DocumentType): boolean {
    ...
    const userDeptId = employees.find(
    ...
    const match = sharedWith.some((entry) => {
      ...
      if (
        entry.type === "employee" &&
        Number(entry.id) === Number(currentUserId)
      )
        return true;
      if (
        entry.type === "department" &&
        userDeptId &&
        Number(entry.id) === Number(userDeptId)
      )
        return true;
      return false;
    });
  ...
  const pendingDocuments = documentsToShow
    .filter((doc) => {
      ...
      const shouldInclude =
        needsReview && isPending && (isUploader || isRecipient);
    ...
    .map((doc) => ({
      ...
      dueDate: new Date(
        new Date(doc.created_at).getTime() + 7 * 24 * 60 * 60 * 1000
      ),
      priority: "high",
      uploaded_by: doc.uploaded_by,
  ...
  const _companyPolicies = documents
        ...
        <div>
          ...
          <p className="text-gray-500">
            {isAdmin
              ...
              : "Access and view shared documents."}
          </p>
        </div>
          ...
          <CardContent className="pt-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-semibold">
                  {
                    documentsToShow.filter((doc) => doc.status === "signed")
                      .length
                  }
                </p>
              </div>
                      ...
                      <div className="flex items-center justify-between p-4 bg-yellow-50 rounded-lg">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            {doc.category} • Due: {formatDate(doc.dueDate)}
                          </div>
                          <div className="text-xs text-gray-400">
                            Shared by {doc.uploaded_by_name}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-row justify-end gap-2 mt-2">
                          {showRejectReason ? (
                            <Button
                              variant="destructive"
                              disabled={
                                reviewLoading || !rejectionReason.trim()
                              }
                              onClick={async () => {
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right text-sm text-gray-500">
                            Updated{" "}
                            {policy.last_updated
                              ? formatDate(new Date(policy.last_updated))
                              : ""}
                          </div>
                          {policy.document_url && (
                            <a
                    ...
                    <div className="h-32 border rounded p-2 overflow-y-auto">
                      {filteredDepartments.map((dept) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={shareFormData.recipients.some(
                              (r) =>
                                r.id === String(dept.id) &&
                                r.type === "department"
                            )}
                      ...
                      {filteredEmployees.map((emp) => (
                        ...
                        >
                          <input
                            ...
                            checked={shareFormData.recipients.some(
                              (r) =>
                                r.id === String(emp.id) && r.type === "employee"
                            )}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setShareFormData((prev) => ({
                  ...
                  <div>
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        className="rounded"
                      />
                      Require Review (document will appear in Pending Actions
                      for recipients)
                    </label>
                  </div>
            ...
            <form onSubmit={handleUploadSubmit}>
              <div className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <Button type="submit" disabled={!uploadFormData.file}>
                    Upload Document
                  </Button>
                </div>
              </div>
