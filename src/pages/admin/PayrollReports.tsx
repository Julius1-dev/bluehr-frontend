...
export function PayrollReports() {
  ...
  const _EXPORT_API = `${BACKEND_URL}/company-admin/payroll-reports/export`;
  const _EMPLOYEE_API = `${BACKEND_URL}/company-admin/employees`;
  const _PAYROLL_API = `${BACKEND_URL}/company-admin/payrolls`;
  ...
  const fetchPayslips = async () => {
    try {
      ...
      if (selectedMonth === "all") {
        ...
        const targetPayroll = payrollHistory.find(
          (payroll) =>
            payroll.payroll_month === selectedMonth &&
            payroll.payroll_year === parseInt(selectedYear)
        );
        targetPayrollId = targetPayroll
          ? targetPayroll.id
          : payrollHistory[0].id;
      }
      ...
      const detailsResponse = await fetch(
        `${BACKEND_URL}/company-admin/payroll/details/${targetPayrollId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const downloadPayrollReportsPDF = async () => {
    ...
    try {
      ...
      doc.autoTable({
        startY: y,
        head: [
          ["Period", "Type", "Total Amount", "Employees", "Processed Date"],
        ],
        body: reportsData,
        theme: "grid",
  ...
  const downloadPayslipsPDF = async () => {
    ...
    try {
      ...
      doc.setTextColor(100, 100, 100);
      doc.text(
        `${selectedMonth} ${selectedYear} • Generated on ${formatDate(
  ...
  const downloadStatutoryPDF = async () => {
    ...
    try {
      ...
      doc.autoTable({
        startY: y,
        head: [
          ["Period", "PAYE", "SHIF", "NSSF", "Housing Levy", "Total", "Status"],
        ],
  ...
  const filteredReports = payrollReports.filter(
    (report) =>
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(v) =>
          setActiveTab(v as "reports" | "payslips" | "statutory")
        }
        className="space-y-4"
      >
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  onClick={downloadPayrollReportsPDF}
                  disabled={downloading || payrollReports.length === 0}
                >
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  No payroll reports available. Process payroll first to
                  generate reports.
                </div>
              )}
            </CardContent>
        ...
        <TabsContent value="statutory" className="space-y-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button
                onClick={downloadStatutoryPDF}
                disabled={downloading || statutoryData.length === 0}
              >
                <Download className="mr-2 h-4 w-4" />
                {downloading ? "Generating..." : "Download PDF"}
              </Button>
                    ...
                    {statutoryData.map((stat, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              stat.status === "paid" ? "default" : "secondary"
                            }
                          >
                            {stat.status}
                          </Badge>
