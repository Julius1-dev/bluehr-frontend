...
export default function ImportPreview() {
  ...
  const { csvData = [], csvHeaders: _csvHeaders = [] } = location.state || {};
  ...
  if (!csvData.length) {
    return (
      <div className="p-8">
        No data to preview.{" "}
        <Button onClick={() => navigate("/admin/team")}>Back</Button>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-8 px-4 max-w-6xl relative">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-32">
        {csvData.map((employee: any, idx: number) => (
          <Card
            key={idx}
            className={
              employee._deptError || rowErrors[idx] ? "border-red-500" : ""
            }
          >
            <CardHeader>
              <CardTitle>
            ...
            <CardContent className="space-y-2">
              {employee._deptError && (
                <div className="text-red-600 font-semibold">
                  Department not found: "{employee.department_name}"
                </div>
              )}
              {rowErrors[idx] && (
                <div className="text-red-600 font-semibold">
                  Upload error: {rowErrors[idx]}
                </div>
              )}
              <div>
              ...
              <div>
                <strong>KRA PIN:</strong> {employee.kra_pin}
              </div>
              <div>
                <strong>NSSF Number:</strong> {employee.nssf_number}
              </div>
              <div>
                <strong>NHIF Number:</strong> {employee.nhif_number}
              </div>
              <div>
                <strong>Joining Date:</strong> {employee.joining_date}
              </div>
              <div>
                <strong>Department Name:</strong> {employee.department_name}
              </div>
              <div>
              ...
              <div>
                <strong>Payment Frequency:</strong> {employee.payment_frequency}
              </div>
              <div>
                <strong>Basic Salary:</strong> {employee.basic_salary}
              </div>
              <div>
                <strong>Bank Name:</strong> {employee.bank_name}
              </div>
              <div>
                <strong>Bank Branch:</strong> {employee.bank_branch}
              </div>
              <div>
                <strong>Account Number:</strong> {employee.account_number}
              </div>
              <div>
                <strong>Account Name:</strong> {employee.account_name}
              </div>
              <div>
              ...
              <div>
                <strong>Status:</strong> <Badge>{employee.status}</Badge>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white border-t py-4 flex justify-center z-50">
        <Button
          ...
          disabled={loading || hasDeptError || hasRowErrors}
        >
