...
export function CompanyProfile() {
  ...
  const renderField = (label: string, value: string | undefined, name: keyof CompanyInfo, type = 'text') => {
    if (name === 'employees') {
      return (
        <div className="space-y-1.5">
          <label className="text-sm font-medium text-gray-500">{label}</label>
          <div className="text-foreground font-semibold">
            {employeeCount !== null ? employeeCount : <span className="text-gray-400">-</span>}
          </div>
        </div>
      );
    }
    return (
      <div className="space-y-1.5">
        ...
        {isEditing ? (
          ...
          <div className="text-foreground">{value || <span className="text-gray-400">-</span>}</div>
        )}
      </div>
    );
  };

  if (loading) return <div className="p-8 text-center text-gray-500">Loading company profile...</div>;
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              {renderField('Company Name', editData.company_name, 'company_name')}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {renderField('Number of Employees', editData.employees, 'employees')}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {renderField('Postal Code', editData.postal_code, 'postal_code')}
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              {renderField('Contact Name', editData.contact_name, 'contact_name')}
              {renderField('Email', editData.contact_email, 'contact_email', 'email')}
              {renderField('Phone', editData.contact_phone, 'contact_phone', 'tel')}
