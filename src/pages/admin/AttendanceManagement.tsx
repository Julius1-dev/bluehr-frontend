...
export function AttendanceManagement(): JSX.Element {
  ...
  const getDateRangeFromFilter = (
    ...
    switch (filter) {
      ...
      case "Last Week": {
        ...
        const diffLastWeek =
          today.getDate() - dayOfLastWeek + (dayOfLastWeek === 0 ? -6 : 1) - 7; // Subtract 7 days
  ...
  const fetchData = async () => {
    try {
      ...
      const needsDateRange = ["This Week", "Last Week", "This Month"].includes(
        selectedDate
      );
      ...
      const [
        ...
        attendanceApi.getEmployeeAttendanceRecords(
          date,
          departmentId,
          startDate,
          endDate
          ...
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-sm">
              {selectedDate === "Today" || selectedDate === "Yesterday"
                ...
                ? `${getDateRangeFromFilter(selectedDate).startDate} to ${
                    getDateRangeFromFilter(selectedDate).endDate
                  }`
                : selectedDate === "This Month"
                ? new Date().toLocaleDateString("en-US", {
                    year: "numeric",
