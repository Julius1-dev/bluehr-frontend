    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        const mapped = (results.data as any[]).map((row) => {
          ...
          const deptName =
            camelRow.departmentName ||
            camelRow.department_name ||
            camelRow.department;
        ...
        navigate("/admin/team/import/preview", {
          state: { csvData: mapped, csvHeaders: results.meta.fields },
        });
      },
    });
  };
  ...
  const handleExport = () => {
    ...
    const tableRows = filteredTeamMembers.map((member) => [
      ...
      departmentsList.find(
        (d) => d.id?.toString() === member.department_id?.toString()
      )?.name || "",
      member.role || "",
      member.status || "",
      member.joining_date
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className="max-w-lg bg-white">
          ...
          <ul className="list-disc ml-6 text-sm mb-2">
            <li>
              Download the sample CSV file and fill in your employee data.
            </li>
            <li>
              Required fields:{" "}
              <b>
                first_name, last_name, email, department_name, role,
                joining_date, status
              </b>{" "}
              (others optional).
            </li>
            <li>
              Department name must match exactly as created in your system.
            </li>
            <li>Upload the completed CSV using the file input below.</li>
          </ul>
          <Button
            className="bg-blue-700 text-white hover:bg-blue-700 hover:text-white"
            size="sm"
            variant="outline"
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4">
          ...
          <Button size="sm" className="gap-1" onClick={handleAddEmployee}>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="border rounded-md overflow-hidden">
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[200px] md:w-[250px]">
                      Employee
                    </TableHead>
                ...
                <TableBody>
                  {filteredTeamMembers.map((member) => {
                    const department = departmentsList.find(
                      (d) =>
                        d.id?.toString() === member.department_id?.toString()
                    );
                    return (
                          ...
                          <DropdownMenu>
                            ...
                            >
                              ...
                              <DropdownMenuItem
                                ...
                                onClick={() =>
                                  navigate(`/admin/team/view/${member.id}`)
                                }
                              >
                              ...
                              <DropdownMenuItem
                                ...
                                onClick={() =>
                                  navigate(
                                    `/admin/team/${member.id}/department`
                                  )
                                }
                              >
                              ...
                              <DropdownMenuItem
                                className="cursor-pointer hover:underline hover:underline-offset-2"
                                onClick={() =>
                                  navigate(`/admin/team/${member.id}/schedule`)
                                }
                              >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mt-4 text-sm text-gray-500 gap-2">
            <div>
              Showing {filteredTeamMembers.length} of {teamMembers.length} team
              members
            </div>
