...
import { Plus, Megaphone, FileText, AlertCircle, Calendar } from "lucide-react"; // Removed unused Clock and X imports
...
export function Announcements() {
  ...
  const handleDeletePolicy = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/company-admin/policies/${deletePolicy.id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleEditPolicy = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/company-admin/policies/${editPolicy.id}`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
      ...
      if (data.success) {
        setPolicies(
          policies.map((p) => (p.id === editPolicy.id ? data.policy : p))
        );
        setEditPolicy(null);
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">
            Manage company-wide communications and policies
          </p>
        </div>
      </div>
          ...
          >
            <DialogContent>
              ...
              <div className="space-y-2">
                <div className="text-xs text-gray-500">
                  Category: {viewPolicy?.category} | Version:{" "}
                  {viewPolicy?.version} | Last Updated:{" "}
                  {viewPolicy?.last_updated
                    ? new Date(viewPolicy.last_updated).toLocaleDateString()
                    : ""}
                </div>
