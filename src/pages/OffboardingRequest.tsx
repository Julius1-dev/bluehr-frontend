...
export default function OffboardingRequest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Your offboarding request has been submitted successfully. HR will review your request and get back to you soon.');
      navigate('/');
    } catch (err) {
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold mb-2">Submit Offboarding Request</h1>
          <p className="text-muted-foreground">Please fill out the form below to submit your offboarding request</p>
        </div>

        <Card className="mb-8">
          <CardHeader>
            <CardTitle>Offboarding Details</CardTitle>
            <CardDescription>Provide the details of your offboarding request</CardDescription>
          </CardHeader>
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Select 
                    value={formData.type} 
                    onValueChange={(value: OffboardingType) => handleChange('type', value)}
                  >
                    <SelectTrigger>
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        onSelect={(date) => handleChange('lastWorkingDay', date || undefined)}
                        initialFocus
                        disabled={(date) => {
                          // Disable past dates
              ...
              <div className="p-4 bg-amber-50 rounded-md flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700">
                    ...
                    Please ensure all your tasks are completed and handover is properly done before your last working day.
                  </p>
                </div>
              </div>
            </CardContent>
            <CardFooter className="flex justify-end gap-4">
              ...
              <Button 
                type="submit" 
                disabled={!formData.lastWorkingDay || !formData.reason || isSubmitting}
              >
          ...
          <CardContent>
            <ul className="space-y-3 text-sm text-muted-foreground">
              <li className="flex items-start gap-2">
                <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-primary font-medium text-xs">1</span>
                <span>Your request will be reviewed by the HR department</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-primary font-medium text-xs">2</span>
                <span>You'll receive a confirmation email with next steps</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-primary font-medium text-xs">3</span>
                <span>HR will schedule an exit interview if required</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-primary font-medium text-xs">4</span>
                <span>Complete the offboarding checklist before your last day</span>
              </li>
            </ul>
          </CardContent>
        </Card>
